...
export async function listStoredAdminUsers() {
  try {
    return supabaseRequest("admin_users?select=username,role,permissions,is_active,created_at,updated_at&order=created_at.asc") || [];
  } catch (error) {
...
export async function listBuyerProfiles() {
  return supabaseOptionalRequest("buyer_profiles?select=*&order=updated_at.desc,created_at.desc&limit=200", []);
}

export async function getBuyerProfile(id) {
  const result = await supabaseRequest(`buyer_profiles?select=*&id=eq.${encodeURIComponent(id)}&limit=1`);
  return result?.[0] || null;
}

export async function insertBuyerProfile(data) {
  return supabaseRequest("buyer_profiles", {
    method: "POST",
    headers: { Prefer: "return=representation" },
    body: JSON.stringify({
      ...
      buyer_stage: data.buyer_stage || "New",
      relationship_status: data.relationship_status || "Prospect",
...
export async function updateBuyerProfile(id, updates) {
  return supabaseRequest(`buyer_profiles?id=eq.${encodeURIComponent(id)}`, {
    method: "PATCH",
    headers: { Prefer: "return=representation" },
...
export async function deleteBuyerProfile(id) {
  return supabaseRequest(`buyer_profiles?id=eq.${encodeURIComponent(id)}`, {
    method: "DELETE"
  });
}
...
export async function listSuppliers() {
  return supabaseOptionalRequest("suppliers?select=*&order=updated_at.desc,created_at.desc&limit=200", []);
}

export async function getSupplier(id) {
  const result = await supabaseRequest(`suppliers?select=*&id=eq.${encodeURIComponent(id)}&limit=1`);
  return result?.[0] || null;
}

export async function insertSupplier(data) {
  return supabaseRequest("suppliers", {
    method: "POST",
    headers: { Prefer: "return=representation" },
    body: JSON.stringify({
...
export async function updateSupplier(id, updates) {
  return supabaseRequest(`suppliers?id=eq.${encodeURIComponent(id)}`, {
    method: "PATCH",
    headers: { Prefer: "return=representation" },
    body: JSON.stringify({ ...updates, updated_at: new Date().toISOString() })
  });
...
export async function listBusinessDocuments() {
  return supabaseOptionalRequest("business_documents?select=*&order=updated_at.desc,created_at.desc&limit=200", []);
}

export async function getBusinessDocument(id) {
  const result = await supabaseRequest(`business_documents?select=*&id=eq.${encodeURIComponent(id)}&limit=1`);
  return result?.[0] || null;
}

export async function insertBusinessDocument(data) {
  return supabaseRequest("business_documents", {
    method: "POST",
    headers: { Prefer: "return=representation" },
    body: JSON.stringify({
...
export async function updateBusinessDocument(id, updates) {
  return supabaseRequest(`business_documents?id=eq.${encodeURIComponent(id)}`, {
    method: "PATCH",
    headers: { Prefer: "return=representation" },
    body: JSON.stringify({ ...updates, updated_at: new Date().toISOString() })
  });
...
export async function deleteBusinessDocument(id) {
  return supabaseRequest(`business_documents?id=eq.${encodeURIComponent(id)}`, {
    method: "DELETE"
  });
}
...
export async function getStoredAdminUserForAudit(username) {
  ...
  let result;
  try {
    result = await supabaseRequest(`admin_users?select=username,role,permissions,is_active,created_at,updated_at&username=eq.${encodeURIComponent(username)}&limit=1`);
  } catch (error) {
    if (!String(error?.message || "").includes("permissions")) {
      throw error;
    }
    result = await supabaseRequest(`admin_users?select=username,role,is_active,created_at,updated_at&username=eq.${encodeURIComponent(username)}&limit=1`);
  }
  return result?.[0] || null;
}
...
export async function updateStoredAdminUser(username, updates) {
  return supabaseWriteWithColumnFallback(`admin_users?username=eq.${encodeURIComponent(username)}`, {
    method: "PATCH",
    headers: { Prefer: "return=representation" },
    body: JSON.stringify({
      ...updates,
...
export async function insertAiAuditLog(data) {
  return supabaseRequest("ai_audit_log", {
    method: "POST",
    headers: { Prefer: "return=representation" },
    body: JSON.stringify({
      user_id: data.user_id || null,
      admin_username: String(data.admin_username || "").slice(0, 120),
      user_role: String(data.user_role || "").slice(0, 80),
      tool_name: String(data.tool_name || "").slice(0, 120),
